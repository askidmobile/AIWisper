...
- **Screen Recording Permission Check**: Приложение теперь проверяет разрешение Screen Recording перед началом записи системного аудио
  ...
  - При отсутствии разрешения показывается диалог с опциями:
    ...
    - Отключить запись системного аудио (записывать только микрофон)
    - Отмена
...
### Fixed
- **Voiceprints Saving**: Исправлена проблема, когда сохранённые голоса не появлялись в настройках и не создавалась директория `voiceprints`.
  - **Проблема**: Директория `voiceprints` не создавалась автоматически при первом запуске.
  - **Решение**: Добавлена проверка и создание директории при инициализации `VoicePrintMatcher`.
- **Retroactive Voiceprint**: Добавлена возможность сохранять голос ("Запомнить голос") для уже записанных сессий.
  - **Функция**: При переименовании спикера с флагом сохранения, система теперь автоматически извлекает аудио этого спикера из записи, генерирует новый голосовой слепок (embedding) и сохраняет его в базу. Раньше это работало только для текущей (активной) записи.
...
- `rust/crates/aiwisper-ml`:
  - Введена константа `THRESHOLD_SESSION = 0.55`
  - `SessionSpeakerRegistry` теперь использует этот порог для матчинга
...
### Fixed
- **VAD детекция тишины**: ретранскрипция и запись теперь используют Silero VAD для решения о тишине.
- **RMS порог**: снижён до 0.008 и используется как fallback, чтобы не терять тихую речь.

## [2.0.30] - 2026-01-15

### Fixed
- **Предупреждения сборки**: исправлены `unused_assignments` и `dead_code` в backend.
- **ts-rs warnings**: TypeScript биндинги активируются только при `--features ts-bindings`.
...
### Fixed
- **Незаполненные транскрипты после записи**: статусы чанков теперь сохраняются как `completed` в файлы и событиях, UI больше не фильтрует готовые отрывки как незавершённые; поле `excluded` также пишетcя на диск для консистентности.

### Changed
- Версия приложения обновлена до 2.0.28.
