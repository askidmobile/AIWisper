/**
 * ProvidersContext - Provides synchronized STT and LLM provider settings
 * 
 * This context ensures all components share the same provider settings state.
 * When settings are updated in one component (e.g., SettingsPage), 
 * all other components (e.g., TranscriptionView) see the updated values.
 */
...
export const ProvidersProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const loadSettings = async () => {
      ...
      try {
        ...
        if (!tauriReady || !tauriInvoke) {
          console.warn('[ProvidersContext] Tauri not available, using defaults');
        ...
        try {
          ...
          if (stt) {
            console.log('[ProvidersContext] Loaded STT settings:', stt.activeProvider);
            setSttSettingsState(stt);
          }
        } catch (e) {
          console.error('[ProvidersContext] Failed to load STT settings:', e);
        }
        ...
        try {
          ...
          if (llm) {
            console.log('[ProvidersContext] Loaded LLM settings:', llm.activeProvider);
            setLlmSettingsState(llm);
          }
        } catch (e) {
          console.error('[ProvidersContext] Failed to load LLM settings:', e);
        }
        ...
        try {
          ...
          console.error('[ProvidersContext] Failed to load providers status:', e);
        }
        
        setIsInitialized(true);
      } catch (e) {
        console.error('[ProvidersContext] Initialization failed:', e);
  ...
  const setSttSettings = useCallback(async (settings: STTProvidersSettings) => {
    ...
    try {
      ...
      console.log('[ProvidersContext] Updated STT settings:', settings.activeProvider);
  ...
  const setLlmSettings = useCallback(async (settings: LLMProvidersSettings) => {
    ...
    try {
      ...
      console.log('[ProvidersContext] Updated LLM settings:', settings.activeProvider);
  ...
  const refreshStatus = useCallback(async () => {
    try {
      ...
      console.error('[ProvidersContext] Failed to refresh status:', e);
    }
  }, []);
  ...
  const hasApiKey = useCallback(async (type: 'stt' | 'llm', providerId: string): Promise<boolean> => {
    try {
      ...
      console.error('[ProvidersContext] Failed to check API key:', e);
      return false;
    }
  }, []);
  ...
  const contextValue: ProvidersContextType = {
    // Loading states
    isLoading,
    isInitialized,
...
export function useProvidersContext(): ProvidersContextType {
