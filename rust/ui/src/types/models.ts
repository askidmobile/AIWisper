...
// Режим гибридной транскрипции (слияния результатов двух моделей)
export type HybridMode = 'parallel' | 'confidence';
...
export interface HybridTranscriptionSettings {
    ...
    useLLMForMerge: boolean;             // Использовать LLM для улучшения результата (независимо от режима слияния)
    mode: HybridMode;                    // Режим слияния: parallel (пословное) или confidence (по порогу)
